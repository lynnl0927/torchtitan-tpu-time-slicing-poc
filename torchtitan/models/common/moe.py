...
class TokenChoiceTopKRouter(Module):
  """This class implements token-choice routing.

  In token-choice top-K routing, each token is

      routed to top K experts based on the router scores.

  Optionally supports node-limited (group-limited) routing where experts are
  divided into groups
  (e.g., by node), and only num_limited_groups groups are considered before
  selecting top_k experts.
  ...
  def forward(
    ...
    _, selected_experts_indices = torch.topk(
        # TODO(b/510002414): Force sorted=True for TorchTPU Aten kernel compatibility.
        scores_for_choice,
        k=self.top_k,
        dim=-1,
        sorted=True,
