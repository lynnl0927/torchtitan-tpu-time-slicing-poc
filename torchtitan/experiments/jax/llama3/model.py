...
class Attention(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.wq = nnx.Linear(
            ...
            use_bias=False, dtype=jnp.bfloat16, param_dtype=jnp.bfloat16,
            rngs=rngs,
        )
        self.wk = nnx.Linear(
    ...
    def __call__(
        ...
        # Expand KV heads for GQA: [batch, seq, n_kv_heads, head_dim]
        # → [batch, seq, n_heads, head_dim]
        k = jnp.repeat(k, self.n_rep, axis=2)
        v = jnp.repeat(v, self.n_rep, axis=2)

        if self._attn_fn is not None:
            # Custom kernel (e.g., splash attention): expects [batch, heads, seq, head_dim]
...
class TransformerBlock(nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.attention = Attention(args, attn_fn=attn_fn, rngs=rngs)
        self.feed_forward = FeedForward(args, rngs=rngs)
        self.attention_norm = RMSNorm(args.dim, eps=args.norm_eps, rngs=rngs)
        self.ffn_norm = RMSNorm(args.dim, eps=args.norm_eps, rngs=rngs)
...
class ScannedTransformerBlocks(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        ref = TransformerBlock(args, attn_fn=attn_fn, rngs=rngs)
        ...
        all_blocks = [ref] + [
            TransformerBlock(args, attn_fn=attn_fn, rngs=nnx.Rngs(i))
...
class Transformer(nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.norm = RMSNorm(args.dim, eps=args.norm_eps, rngs=rngs)
